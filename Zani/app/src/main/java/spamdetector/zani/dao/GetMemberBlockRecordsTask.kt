...
class GetMemberBlockRecordsTask : MyAsyncTask<ArrayList<String>, ArrayList<BlockInfo>>() {
    ...
    override fun doInBackground(arg: ArrayList<String>?): ArrayList<BlockInfo>? {
        ...
        var array = ArrayList<BlockInfo>()

        if (con.responseCode == 200) {
            val dataObejct =  JSONObject(builder.toString()).getJSONArray("data")

            for (i: Int in 0 until dataObejct.length()) {
                var blockObject = dataObejct.getJSONObject(i)
                val phoneNum = blockObject.getString("blockNum")
                val date = blockObject.getString("createdAt")
                array.add(BlockInfo(phoneNum = phoneNum, date = date))
            }
        }
        else return null
        return array
